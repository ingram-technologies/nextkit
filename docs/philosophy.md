...
the one `@ingram-tech/nk-db` pool — not on any hosted product. Not every site needs
a database; those that do use our Postgres.
...
- **Auth (when needed):** **Better Auth**, running in our own Next.js process and
  storing in our own Postgres — `@ingram-tech/nk-auth`. No Clerk, no per-MAU SaaS.
- **Dev/test database:** **PGlite** (Postgres-in-WASM, in-process) — no Docker,
  no daemon. `nk dev` boots it and applies migrations; tests run against an
  in-memory instance. See [`db-package.md`](./db-package.md).
