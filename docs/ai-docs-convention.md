...
them). This is a deliberate, systematic pattern — pioneered in `financica` — and
...
- **Subsystem walkthroughs**: how a non-trivial flow works, with the entry-point
  file path and the high-level steps. (financica: `invoice-payment-application.md`.)
- **Architecture decisions**: where new code should live, which patterns to
  phase out. (financica: `architecture-decisions.md`.)
